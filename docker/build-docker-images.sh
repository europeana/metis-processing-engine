...
docker build --no-cache -t flink:1.20.0-java21_poc flink-java21

echo "building :: flink-node $(realpath flink-with-tools)"
docker build --no-cache -t flink-with-tools:1.20.0-java21 flink-with-tools
...
docker build --no-cache -t metis-processing-engine-flink:1.20.0-java21 metis-processing-engine-flink
